...
ENV TOMCAT_VERSION_UPDATE 36
ENV TOMCAT_HOME /usr/tomcat
ENV TOMCAT_FOLDER_NAME apache-tomcat-${TOMCAT_VERSION_MAJOR}.0.${TOMCAT_VERSION_UPDATE}
RUN apt-get update && \
wget -q http://apache.fayea.com/tomcat/tomcat-${TOMCAT_VERSION_MAJOR}/v${TOMCAT_VERSION_MAJOR}.0.${TOMCAT_VERSION_UPDATE}/bin/apache-tomcat-${TOMCAT_VERSION_MAJOR}.0.${TOMCAT_VERSION_UPDATE}.tar.gz
